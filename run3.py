...
def evalacc(model, devloader, test_set):
    ...
    for i in range(1, 10, 1):
        ...
        getRecall(predtrue, negativemask, resp, test_set)
    ...
    recall = getRecall(pred1, negativemask, resp, test_set)
...
def getRecall(realpred, negativemask, resp, test_set):
    ...
    #print(TP, FP, len(test_set) - negativemask.sum(), TN)
    ...
    for epoch in range(500):
        ...
        for dBatch in tqdm(data_loader):
            ...
            if j % 3000 == 0:
                ...
                acc = evalacc(model, devloader, test_set)#scores[0]
                #acc1 = evalacc(model, valloader, val_set)
                print('current accuracy and epoch (%f, %f) max acc and best epoch  (%f, %f) '%(acc, epoch, maxAcc, betterepoch))
                #assert(0)
                if epoch%10 == 0:
                    with open("finalresult/%s_%s_2.txt"%(patchdir,str(testnum)), mode='a+', encoding='utf-8') as file_obj:
                        file_obj.write(str(acc) + ',' + str(epoch)+ '\n')

               
                if maxAcc < acc:
                    maxAcc = acc
                    betterepoch = epoch
                    print("find better acc %s and epoch %s " + str(maxAcc) + ' ' + str(betterepoch))
                    with open("finalresult/%s_%s_2.txt"%(patchdir,str(testnum)), mode='a+', encoding='utf-8') as file_obj:
